...
- Loader respects a minimum visible duration of 1.25s in standard motion and about 150ms when reduced motion is preferred, then fades over 0.42s; grid dispersal delay is 0.02s so loader fade overlaps stack reveal — feels like one connected sequence
- As the loader fades, 16 poster frames (desktop) or 15 (mobile) appear stacked one on top of the other in the center of the page (starting scale 0.12, opacity, centered <300x<150) then disperse in specific radial directions toward destination locations: 4 persistent cards travel to exact grid-path positions, 12 temporary frames fan radially offscreen (angle 2.18 rad, distance 1.28x viewport, fading opacity) over 0.95-1.15s; continuous grid motion begins after 2.85s entrance window without layer swap, preserving same DOM elements (same continuity token), creating one connected loader-fade → stack → dispersal → drift sequence
- Project tile click gradually brings tile to center via shared layoutId `project-{id}`, enlarges to modal envelope (82vw/72vh), displayed on top of existing content with grid visible in background (translucent white — 16% on desktop, 82% on small screens — plus 3px blur), not hiding tiles; scrolling to bottom of modal content auto-closes (wheel accumulator >90 or scroll near bottom) and hero image shrinks back to click location via layoutId reverse
- About surface slides upward from below the viewport over 750ms
- Continuous drift, hover enlargement, and ring parallax stop when reduced motion is preferred: static grid of 4 desktop / 3 mobile cards with discrete Prev/Next buttons plus Arrow-key navigation cycling through all 17 projects (wheel/touch scrolling not hijacked), static list showing all titles with Prev/Next and arrow-key navigation; durations become effectively instant
