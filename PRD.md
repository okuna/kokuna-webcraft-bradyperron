...
- Loader respects a minimum visible duration of 1.25s in standard motion and about 150ms when reduced motion is preferred, then fades over 0.42s
- The loader fade overlaps the stack reveal so the entrance feels like one connected sequence
- As the loader fades, 16 poster frames on desktop or 15 on mobile appear stacked one on top of the other in the center of the page, then disperse in specific radial directions toward their destination locations
- Four persistent cards travel to their exact grid-path positions while 12 temporary frames fan radially offscreen and fade over about one second
- Continuous grid motion begins after the 2.85s entrance window with no visible reset, completing one connected loader-fade, stack, dispersal, and drift sequence
- Project tile click gradually moves the tile to the center of the page and enlarges it to the modal media envelope (82vw by 72vh), displayed on top of existing content with the grid still visible behind a translucent white scrim and soft blur
- Scrolling to the bottom of the modal content automatically closes the dialog, and the hero image shrinks back to the tile location it was opened from
- About surface slides upward from below the viewport over 750ms
- When reduced motion is preferred, continuous drift, hover enlargement, and ring parallax stop: a static grid of 4 desktop or 3 mobile cards offers discrete Previous/Next buttons plus arrow-key navigation cycling through all 17 projects, and page scrolling behaves normally
- Reduced-motion list view shows all titles with Previous/Next controls and arrow-key navigation, and only visible controls accept keyboard focus; durations become effectively instant
